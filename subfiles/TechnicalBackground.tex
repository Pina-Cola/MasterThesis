...
\begin{table}[H]
	...
	\begin{tabular}{ll}
		...
		sint32 & Signed 32-bit integer (efficient for encoding negative numbers) \\
		sint64 & Signed 64-bit integer (efficient for encoding negative numbers) \\
		fixed32 & Unsigned 32-bit integer (always four bytes) \\
		fixed64 & Unsigned 64-bit integer (always eight bytes) \\
		sfixed32 & Signed 32-bit integer (always four bytes) \\
		sfixed64 & Signed 64-bit integer (always eight bytes) \\
	...
	\caption{Data types for fields in Protocol Buffers (proto2)}
	\label{table:proto2_data_types}
\end{table}


%Common data types supported for fields include integers (int32, int64), floating-point numbers (float, double), strings, booleans, and byte arrays.


%\textbf{Enum Definition:}
%\begin{itemize}
%	\item Additionally, proto2 supports the definition of enumeration types (enums).
%	\item Enums allow for the declaration of a set of named constants, each associated with a unique integer value.
%	\item Enums are useful for representing a fixed set of options or states within a message definition.
%\end{itemize}
