...
In the following, a selection of filters, that influence the RGB representation of the video are listed. This includes the filter name, description and possible parameters from the Melt website.\myfootcite{melt_filters} Only the relevant parameters will be listed.
...
In addition to this, it needs to be tested, if multiple parameters of the same filter can be applied simultaneously. For evaluating, the filters and their different parameters are all applied to the same frame of the test video file. The original frame $343$ can be seen in Figure \ref{figure:nofilter}.

\begin{figure}[H]
	\begin{center}
		\cutpic{0.3cm}{0.7\textwidth}{nofilter.png}
...
The parameters of this filter take input values as float between $-1$ and $1$. It has individual parameters to set the shadows, mid tones and highlights per colour. This can be seen in Figure \ref{figure:rsrmrh}.

\begin{figure}[H]
	\begin{center}
		\cutpic{0.32cm}{0.3\textwidth}{rs.png}
		%\hspace*{0.01\textwidth}
		\cutpic{0.32cm}{0.3\textwidth}{rm.png}
		%\hspace*{0.01\textwidth}
		\cutpic{0.32cm}{0.3\textwidth}{rh.png}
		\small{
