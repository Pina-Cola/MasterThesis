...
This is possible, because the Accurate Player uses JIT to dynamically handle the playback of different video formats on-the-fly, instead of relying on pre-transcoded video files. The video data is being processed in real-time, allowing for on-the-fly adjustment of options such as the quality slider or the RGB colour adjustment that is being implemented for this thesis project.
...
The MLT filters \texttt{avfilter.colorbalance}, \texttt{avfilter\-.colorchannelmixer} and \texttt{frei\-0r\-.coloradj\_RGB} are compared regarding the colour vibrance and intensity of the result. This analysis of the filters above was conducted regarding the colours red and yellow in depth and a side-by-side comparison can be seen in Figure~\ref{figure:redcomp} and~\ref{figure:yellowcomp}. The colour changes of the other two colour channels green and blue work analogically and are displayed in Figure~\ref{figure:filtersG} and \ref{figure:filtersB}.
