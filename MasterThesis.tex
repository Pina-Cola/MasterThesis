...
TODO: umstrukturieren und mehr auf farben beziehen
...
TODO: split up in theoretical background for technical things and \textit{colour theory}?
...
%-------------------------------------------------------------------------------------------------------
\subsection{Named pipi} \label{subsection:namedpipe}

A named pipe, also known as a FIFO (First In, First Out), is a type of interprocess communication (IPC) mechanism used in Unix-like operating systems. It provides a way for two or more processes to communicate with each other, passing data between them. Here are some key characteristics of named pipes:
...
Named pipes follow a FIFO structure, meaning that the first data written into the pipe is the first data to be read. It maintains a sequential order for data transmission.
Named vs. Unnamed Pipes:

Unlike unnamed pipes, which are created using the pipe system call and exist only within the context of the parent-child relationship between processes, named pipes have a persistent existence. They are associated with a filesystem entry, usually represented as a file.
Filesystem Entry:

Named pipes are created as special files in the filesystem. These files have a name associated with them and are stored in a directory, just like regular files. The naming allows unrelated processes to access the same named pipe for communication.
Communication between Unrelated Processes:

Named pipes enable communication between unrelated processes. Processes that are not parent and child can communicate by opening the same named pipe file.
Bidirectional Communication:

Named pipes support bidirectional communication. Once a named pipe is created, processes can open it for reading and writing, allowing data to flow in both directions.
Blocking and Non-blocking Operations:

Operations on named pipes can be blocking or non-blocking. When a process attempts to read from an empty pipe, it may block until data is available. Similarly, when writing to a full pipe, a process may block until there is space.
Lifecycle and Persistence:

Named pipes exist beyond the lifetime of the processes that create them. They are removed from the filesystem only when explicitly deleted or when the system is restarted.
Named pipes are widely used in Unix-like systems for various purposes, such as communication between different applications or components of a system. They provide a simple and efficient means of passing data between processes in a way that is independent of their relationship in the process hierarchy.
...
TODO: Überarbeiten, kurz einleiten und in entsprechende Chapter verschieben
...
JIT-WebRTC is a live transcoder with a WebRTC output that is run in a docker container. As described in Section~\ref{subsection:videotranscoding}, transcoding is the conversion of one digital data format into another.~\cite{transcoding}
...
Control commands are sent from the browser to \texttt{main.py} with a WebRTC data channel.
...
Melt sends an AVI stream with the rendered video and audio to the \texttt{main.py}.
AVI is a file format that can contain audio and video information to allow synchronized playback of audio and video components. 
It is a container format, which means that it can contain multiple streams of audio and video data, along with other multimedia data such as subtitles.~\cite{avi} AVI was further described in Section~\ref{subsection:avi}.
...
Was soll diese ganze subsection eigentlich? Ich denke die kann weg aber mal sehen.
...
\includegraphics[page=1, width=0.91\textwidth]{FE.pdf}

\includegraphics[page=2, width=0.91\textwidth]{FE.pdf}
...
\includegraphics[page=1, width=0.91\textwidth]{BE.pdf}

\includegraphics[page=2, width=0.91\textwidth]{BE.pdf}
